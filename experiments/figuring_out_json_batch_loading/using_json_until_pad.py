...
that will need to deal with our custom json structure.
"""
...
def _build_seq_tensor(patient, sequence_length):
    ...
    for episode_i, episode in enumerate(patient["episodes"]):
        ...
        for event in clinical_events:
            ...
            if "entries" in event:
                ...
                # raise ValueError('double check that indexes_category_counts is what you think it is')
                        ...
                        if running_numeric_entries_total < config.encoder.ndim_dict["num_s"] - 1:
                            try:
                                ...
                                values_numeric["values"].append(
                                    0.0
                                )  # ehr-predictions readme states this is the numerical imputed value for missing entries
    ...
    # all sequences need to have same 1st dimension for dense embedding to work, so get the max 1st dimension
    # in order to build tensors later
    max_shape_1 = 0
    for key in sequences.keys():
        if len(sequences[key]["indices"]) > 0:
            curr_shape_1 = (np.max(sequences[key]["indices"], axis=0) + 1)[1]
            if curr_shape_1 > max_shape_1:
                max_shape_1 = curr_shape_1
    ...
    for key in sequences.keys():
        ...
        if len(sequences[key]["indices"]) == 0:
            ...
            sequences[key]["dense_shape"][1] = max_shape_1
            expected_dims = None
...
batch_generator = create_batch_generator(data_generator, batch_size=128)
dataset = tf.data.Dataset.from_generator(batch_generator, output_types=output_types, output_shapes=output_shapes)

dataset = dataset.map(
    lambda x: tf_dataset_utils.convert_required_tensors_to_sparse(x, tensor_to_sparse_keys), num_parallel_calls=128
)
