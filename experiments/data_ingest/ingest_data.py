...
def extract_feature_mapping(artifacts_dir, debug_dir, args, data_index, raw_data, root_logger):
...
def generate_label_template(data_index):
    ...
    # TODO: Generate temporal labels - add to labelling below
...
def main(args):  # noqa: C901
    ...
    artifacts_dir = Path(args[0]).parent / (str(Path(args[0]).stem) + "_" + timestamp)
    ...
    root_logger.info(f"Processing: {Path(args[0])}")
    ...
    save_dictionary_json(artifacts_dir / (str(Path(args[0]).stem) + ".json"), data_index)
    ...
    (
        ...
        numerical_feature_mapping,
    ) = extract_feature_mapping(artifacts_dir, debug_dir, args, data_index, raw_data, root_logger)
    ...
    for i, master_key in enumerate(tqdm(unique_master_keys)):
            ...
            if len(master_key_slice[value["field"]].values):
                if value["type"] == "numerical":
                    ...
                    if str(master_key_slice[value["field"]].values[0]) != "nan":
                        ...
                        for code in categorical_value.split(","):
        ...
        for event_def in data_index["mapping"]["events"]:
            ...
            for event in events.itertuples():
                ...
                if pd.isnull(timestamp) or str(timestamp) == "nan" or timestamp is None or timestamp == "None":
                    ...
                    year_of_birth = int(record[str(metadata_mapping["year_of_birth"])])
        ...
        record["episodes"][0]["events"] = label_events(
