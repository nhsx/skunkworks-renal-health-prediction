...
def main(args):  # noqa: C901
    ...
    file_handler = logging.FileHandler(
        "{0}/{1}.log".format(artifacts_dir, f"{timestamp}_normalisation_log_uncapped.txt")
    )
    ...
    for i, record in enumerate(tqdm(data)):
        ...
        output_normalised_location = artifacts_dir / "ingest_records_output_lines_normalised_uncapped.jsonl"
        append_jsonl(output_normalised_location, updated_record)

        # Export subset of structured records
        if i < 10:
            output_temp_location = debug_dir / f"ingest_records_output_single_line_{i}_normalised_uncapped.json"
